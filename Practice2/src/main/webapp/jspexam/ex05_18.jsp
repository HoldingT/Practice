...
<body>
	...
	if (request.getMethod().equals("POST")){
		if(session.isNew() || session.getAttribute("member_id") == null ){
			String name = request.getParameter("memberpassword");
			if(name.equals("duke") && passwd.eqals("java")){
	...
	%>
				...
				<h3><%= name %>회원님</h3>
				<a href="Practice/jspexam/ex05_19.jsp">로그아웃</a>
			<%
			} else {
			%>
				<script>
	...
	%>		
		...
		%>
			...
			<h2>로그인</h2><hr>
			<form method = "post"	action="Practice/jspexam/ex05_19.jsp">
					<input placeholder="계정을 입력하세요" name="membername" required><br>
					<input type="password" placeholder="암호를 입력하세요" name="memberpassword" required><br>
					<input type="submit" value="로그인">			
			</form>
